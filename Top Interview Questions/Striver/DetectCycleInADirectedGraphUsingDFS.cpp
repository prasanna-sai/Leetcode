...
public:
    ...
    bool helper(int src, vector<vector<int>>& adj, vector<int>& vis, vector<int>& dfsVis)
    {
        ...
        {
            if(!vis[adj[src][i]]) //we can use dfsvis as well, but its slow as we know since it is already visited, in another iteration, no need to visit again eventhough dfsvis is not set.
            {
